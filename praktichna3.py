    ...
    for cnt in contours:
        ...
        if area > 1000:
            ...
            if M["m00"] != 0:
                ...
                t = cv2.circle(frame, (cx, cy), 7, (0, 255, 0), 2)
                points.append((cx, cy))

            if object_detected_this_frame:
                cv2.putText(frame, 'Recording', (0, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                cv2.rectangle(frame, (0, 0), (frame.shape[1], frame.shape[0]), (0, 255, 0), 2)
    #
    # for i in range(1, len(points)):
    #     if points[i - 1] is None or points[i] is None:
    #         continue
    #     cv2.line(frame, points[i - 1], points[1], (0, 0, 255), 2)
    #
